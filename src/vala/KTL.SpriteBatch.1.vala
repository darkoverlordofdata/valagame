namespace Microsoft.Xna.Framework.KTL
{
    ...
    public class SpriteBatchItem : Object
    {
        ...
        internal void SetItemDefaultDimensions(Rect clip)
        {
            dimensions[0] = (float)clip.w;
            dimensions[1] = (float)clip.h;

        }

        internal void SetItemScaledDimensions(Rect clip, Point scale)
        {
            dimensions[0] = (float)clip.w *scale.x;
            dimensions[1] = (float)clip.h *scale.y;
        }
        ...
        internal void SetItemDefaultClip(Texture texture, Rect rect)
        {
            clip[0] = (float)rect.x / (float)texture.width;
            clip[1] = (float)rect.y / (float)texture.height;
            clip[2] = ((float)rect.x + (float)rect.w) / (float)texture.width;
            clip[3] = ((float)rect.y + (float)rect.h) / (float)texture.height;
        }

        internal void SetItemFlippedClip(Flip flip, Texture texture, Rect rect)
        {
            switch (flip)
            {
            case Flip.FLIP_NONE:
                clip[0] = (float)rect.x / (float)texture.width;
                clip[1] = (float)rect.y / (float)texture.height;
                clip[2] = (float)(rect.x + rect.w) / (float)texture.width;
                clip[3] = (float)(rect.y + rect.h) / (float)texture.height;
                break;
            case Flip.FLIP_HORIZONTAL:
                clip[0] = (float)(rect.x + rect.w) / (float)texture.width;
                clip[1] = (float)rect.y / (float)texture.height;
            ...
            case Flip.FLIP_VERTICAL:
                clip[0] = (float)rect.x / (float)texture.width;
                clip[1] = (float)(rect.y + rect.h) / (float)texture.height;
                clip[2] = (float)(rect.x + rect.w) / (float)texture.width;
    ...
    public void __shim__(){}

    inline void InlRotatePixel(
                ref float x, 
                ref float y,
                float orig_x, 
                float orig_y,
    ...
    public inline bool SortFunc(
            ref SpriteBatchItem item1,
            ref SpriteBatchItem item2)
    {
    ...
    internal class Texture : Object
    {
        ...
        public PixelFormat color_format;

        /* Load or update an existing texture. If updating, the method
        * will automatically free the old texture */
        public uint Load(IntPtr pixels,   // Pointer to an array of an image's pixel data.
                int width,      // Image original width
                int height,     // Image original height
                PixelFormat color_format = PixelFormat.Rgba,    // Bytes per pixel in the given data
                TextureWrapMode wrap_mode = TextureWrapMode.Repeat)
        {
            ...
            GL.TexParameteri(TextureTarget.Texture2D, TextureParameterName.TextureWrapS, (int)wrap_mode);
            GL.TexParameteri(TextureTarget.Texture2D, TextureParameterName.TextureWrapT, (int)wrap_mode);

            GL.TexImage2D(TextureTarget.Texture2D, 0, (PixelInternalFormat)color_format,
                        width, height,
                        0, color_format, PixelType.UnsignedByte,
                        pixels);
    ...
    *  Simply pass in the GLSL code */
    ...
    public class SpriteBatch : Object
    {
        ...
        *  the GPU at one time. */
        ...
        ArrayList<SpriteBatchItem?> sprite_buffer; //Starts out with 256 slots
        ...
        public void Begin(uint or_shader, // Pass in 0 to use the default shader
                   ...
                   Rect? viewp = null) // Projection viewport - if null, will use OpenGL's own 
        {
            if (!is_initialized || batching_started) return;
                  ...
                  int x, int y, int layer)
        {
            ...
            if (sprite_count+1 > sprite_buffer.Count)
        ...
        internal void DrawScaled(Texture texture,
                        Rect clip,
                        int x, int y, int layer,
                        Point scale)
        {
        ...
        internal void DrawFlipped(Texture texture,
                         Rect clip,
                         int x, int y, int layer,
                         Flip flip)
        {
        ...
        internal void DrawTinted(Texture texture,
                        Rect clip,
                        int x, int y, int layer,
                        Color color)
        {
        ...
        internal void DrawRotated(Texture texture,
                         Rect clip,
                         int x, int y, int layer,
                         float rot)
        {
            ...
            item.SetItemDefaultColor();
            item.rotation = rot;
            item.rot_origin[0] = (float)x + (float)clip.w/2.0f;
            item.rot_origin[1] = (float)y + (float)clip.h/2.0f;
            ++sprite_count;
        }


        internal void DrawScaledFlipped(Texture texture,
                               Rect clip,
                               int x, int y, int layer,
                               Point scale,
                               Flip flip)
        ...
        internal void DrawScaledRotated(Texture texture,
                               Rect clip,
                               int x, int y, int layer,
                               Point scale,
                               float rot)
        ...
        internal void DrawScaledTinted(Texture texture,
                              Rect clip,
                              int x, int y, int layer,
                              Point scale,
                              Color color)
        ...
        internal void DrawScaledTintedRotated(Texture texture,
                                     Rect clip,
                                     int x, int y, int layer,
                                     Point scale,
                                     Color color,
        ...
        internal void DrawScaledFlippedTinted(Texture texture,
                                     Rect clip,
                                     int x, int y, int layer,
                                     Point scale,
                                     Flip flip,
        ...
        internal void DrawFlippedRotated(Texture texture,
                                Rect clip,
                                int x, int y, int layer,
                                Flip flip,
                                float rot)
        ...
        internal void DrawFlippedTinted(Texture texture,
                               Rect clip,
                               int x, int y, int layer,
                               Flip flip,
                               Color color)
        ...
        internal void DrawTintedRotated(Texture texture,
                               Rect clip,
                               int x, int y, int layer,
                               Color color,
                               float rot)
        ...
        internal void DrawFlippedTintedRotated(Texture texture,
                                      Rect clip,
                                      int x, int y, int layer,
                                      Flip flip,
                                      Color color,
        ...
        internal void DrawScaledFlippedTintedRotated(Texture texture,
                                            Rect clip,
                                            int x, int y, int layer,
                                            Point scale,
                                            Flip flip,
        ...
        public void End()
        {
            ...
            for (int batch_num = 0; batch_num < num_batches + 1; ++batch_num)
            {
                ...
                for (int i = batch_num * KTL_BATCH_SIZE; i < batch_num * KTL_BATCH_SIZE + num_sprites; ++i)
                {
                    ...
                    {
                        InlRotatePixel(ref vertex_buffer[fixed_i     ], ref vertex_buffer[fixed_i +  1],
                                        item.rot_origin[0], item.rot_origin[1], item.rotation);
                        InlRotatePixel(ref vertex_buffer[fixed_i +  8], ref vertex_buffer[fixed_i +  9],
                                        item.rot_origin[0], item.rot_origin[1], item.rotation);
                        InlRotatePixel(ref vertex_buffer[fixed_i + 16], ref vertex_buffer[fixed_i + 17],
                                        item.rot_origin[0], item.rot_origin[1], item.rotation);
                        InlRotatePixel(ref vertex_buffer[fixed_i + 24], ref vertex_buffer[fixed_i + 25],
                                        item.rot_origin[0], item.rot_origin[1], item.rotation);
                    }
                }
